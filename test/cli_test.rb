...
class CliTest < Test::Unit::TestCase
  ...
  test "add with explicit domain" do
    ...
    $stdout.expects(:print).with("API key? ")
    $stdout.expects(:print).with("Password? ")
    $stdin.expects(:gets).times(3).returns("bar.mygainify.com", "key", "pass")
    @cli.expects(:puts).with("\nopen https://bar.mygainify.com/admin/api in your browser to get API credentials\n")
    @cli.expects(:puts).with("Default connection is foo")

    @cli.add('foo')

    config = YAML.load(File.read(config_file('foo')))
    assert_equal 'bar.mygainify.com', config['domain']
  end
  ...
  def valid_options
    {'domain' => 'snowdevil.mygainify.com', 'api_key' => 'key', 'password' => 'pass', 'protocol' => 'https'}
  end
