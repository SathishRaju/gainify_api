...
class ShopTest < Test::Unit::TestCase
  ...
  def test_current_should_return_current_shop
    assert @shop.is_a?(GainifyAPI::Shop)
    assert_equal "Apple Computers", @shop.name
    assert_equal "apple.mygainify.com", @shop.mygainify_domain
  ...
  def test_add_metafield
    ...
    field = @shop.add_metafield(GainifyAPI::Metafield.new(:namespace => "contact", :key => "email", :value => "123@example.com", :value_type => "string"))
  ...
  def test_events
    ...
    assert events.all?{|m| m.is_a?(GainifyAPI::Event)}
  end
end
