  ...
  class Cli < Thor
    ...
    def add(connection)
      ...
      if File.exist?(file)
        ...
        config['domain']   = ask("Domain? (leave blank for #{connection}.mygainify.com)")
        config['domain']   = "#{connection}.mygainify.com" if config['domain'].blank?
        config['domain']   = "#{config['domain']}.mygainify.com" unless config['domain'].match(/[.:]/)
    ...
    def console(connection=nil)
      ...
      GainifyAPI::Base.site = site_from_config(config)
    ...
    def shop_config_dir
      @shop_config_dir ||= File.join(ENV['HOME'], '.gainify', 'shops')
    end
    ...
    def site_from_config(config)
      ...
      GainifyAPI::Base.site = "#{protocol}://#{api_key}:#{password}@#{domain}/admin"
    end
